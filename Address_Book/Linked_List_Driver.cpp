...
int main() {
	...
	while (finished != true) { //get input
		...
		{
			ll.add_first(contact, phone);
			first = false;
		} else { //if it isn't the first node
			ll.add_node_end(contact, phone);
		}
		std::cout << "Enter 1 to Delete 2 to add node and 3 to print out list" << std::endl;
		std::getline(std::cin, op);
		if(op == "1")
		{
			std::cout << "ENter the contact to d"
			continue;
		}else if(op == "2"){
			finished = false;
			continue;
